...
defmodule PerfectSquares do
    ...
    def create_workers(n, k, actorNum, workUnit, numOfWorkers, use_multiple_machines) do
        if actorNum < 1 do 
            ...
            if use_multiple_machines == 1 do
                if rem(n, 2) == 0 do
                    pid = spawn(PerfectSquares, :worker, [actorNum, k, actualWorkUnit, self()])
                    inspect(pid)
                else
                    pid = Node.spawn(:"bhanje@192.168.43.137", PerfectSquares, :worker, [actorNum, k, actualWorkUnit, self()])
                    inspect(pid)
                end
            else
                pid = spawn(PerfectSquares, :worker, [actorNum, k, actualWorkUnit, self()])
